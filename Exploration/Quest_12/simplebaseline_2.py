...
get_ipython().system('mkdir -p ~/aiffel/mpii')
get_ipython().system('ln -s ~/data/* ~/aiffel/mpii')
get_ipython().system('cd ~/aiffel/mpii')
get_ipython().system('tar -xvf mpii_human_pose_v1.tar.gz -C . ')
get_ipython().system('unzip mpii_human_pose_v1_u12_2.zip')


# In[13]:


get_ipython().system('cd ~/aiffel/mpii/mpii_human_pose_v1_u12_2')
get_ipython().system('wget https://d3s0tskafalll9.cloudfront.net/media/documents/train.json')
get_ipython().system('wget https://d3s0tskafalll9.cloudfront.net/media/documents/validation.json')


# In[ ]:


get_ipython().system('cd ~/aiffel/mpii && unzip mpii.zip')
...
# 수정
def train(epochs, learning_rate, num_heatmap, batch_size, train_tfrecords, val_tfrecords, is_Hourglass=True):
    strategy = tf.distribute.MirroredStrategy()
    global_batch_size = strategy.num_replicas_in_sync * batch_size
    train_dataset = create_dataset(
        train_tfrecords, global_batch_size, num_heatmap, is_train=True)
...
get_ipython().run_cell_magic('time', '', "train_tfrecords = os.path.join(TFRECORD_PATH, 'train*')\nval_tfrecords = os.path.join(TFRECORD_PATH, 'val*')\n\n# Constants for hyperparameters\nepochs = 5\nbatch_size = 16\nnum_heatmap = 16\nlearning_rate = 0.0007\n\nbest_simplebaseline_model_file, history_simplebaseline = train(\n    epochs, learning_rate, num_heatmap, batch_size, train_tfrecords, val_tfrecords, is_Hourglass=False)")


# In[ ]:


plot_history(history_simplebaseline, "StackedHourglass")
